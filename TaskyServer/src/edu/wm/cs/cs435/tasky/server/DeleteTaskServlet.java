...
/**
 * Servlet implementation for the delete task functionality
 *
...
public class DeleteTaskServlet extends HttpServlet
{
	...
	{
		response.getOutputStream().println("DeleteTaskServlet.doGet() works! Use the doPost() method from Android or web application");
	}
	...
	protected void doPost(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException
	{
		try
		{
			...
			//the expected request is on 4 lines, and it is similar to reading from a file:
			//DELETE_TASK
			//email
			//projectID
			//taskID
			...
			if (ITaskyServer.DELETE_TASK_OPERATION.equals(requestType)==false)
			{
				throw new IOException("Invalid command. "+ITaskyServer.DELETE_TASK_OPERATION+" expected");
			}
			...
			String taskID=readerFromClient.readLine();			//read fourth line of the request
			...
			String deleteTaskStatus=Server.instance.deleteTask(email, projectID, taskID);
			
			System.out.println("SERVER: server response is");
			System.out.println(deleteTaskStatus);
			...
			writerToClient.write(deleteTaskStatus);
